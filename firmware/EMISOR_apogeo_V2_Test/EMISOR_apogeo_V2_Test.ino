...
#ifndef LED_BUILTIN
#define LED_BUILTIN 2   // La mayoría de las devkits ESP32 traen el LED en GPIO2
#endif
...
// Protocolo de ACK con la estación en tierra: el RSSI/SNR de un paquete solo
// lo conoce quien lo RECIBE, no quien lo transmite. Por eso, al detectar el
// apogeo, esta placa manda el aviso y espera una breve confirmación de la
// estación en tierra con los valores que ella midió al recibirlo.
const unsigned long ACK_TIMEOUT_MS = 1000; // cuánto esperar la respuesta antes de rendirse
...
// Espera hasta ACK_TIMEOUT_MS una respuesta de la estación en tierra con el
// formato "ACK_APOGEO,<rssi>,<snr>". Devuelve true si llegó a tiempo.
bool esperarAckApogeo(float &rssi_out, float &snr_out, unsigned long timeout_ms) {
  ...
  while (millis() - inicio < timeout_ms) {
    ...
    if (tamPaquete) {
      ...
      if (recibido.startsWith("ACK_APOGEO")) {
        int coma1 = recibido.indexOf(',');
        int coma2 = recibido.indexOf(',', coma1 + 1);
        if (coma1 > 0 && coma2 > coma1) {
          rssi_out = recibido.substring(coma1 + 1, coma2).toFloat();
          snr_out = recibido.substring(coma2 + 1).toFloat();
          return true;
        }
      }
    }
...
void notificarApogeo(float altura, bool datosValidos, float rssi, float snr) {
  const char* linea = "**********************************************";
  ...
  if (datosValidos) {
    cartel += "RSSI: " + String(rssi, 0) + " dBm\n";
    cartel += "SNR: " + String(snr, 2) + " dB\n";
  } else {
    cartel += "RSSI: sin confirmacion de tierra\n";
    cartel += "SNR: sin confirmacion de tierra\n";
  }
  cartel += String(linea);
    ...
    case ASCENSO: {
      if (altitud_filtrada > altitud_maxima) {
        ...
        if (contador_apogeo >= MUESTRAS_APOGEO) {
          ...
          float rssiConfirmado = 0, snrConfirmado = 0;
          bool ackRecibido = false;

#if !MODO_SIMULACION
          // Aviso inmediato a tierra, sin esperar el ciclo normal de telemetría
          LoRa.beginPacket();
          LoRa.print("APOGEO," + String(altitud_apogeo, 2));
          LoRa.endPacket();

          // Breve espera (<=1s) por la confirmación con RSSI/SNR de tierra.
          // Este es el único punto del programa donde el loop se pausa,
          // y ocurre justo al empezar el descenso, así que no compromete
          // la detección de aterrizaje.
          ackRecibido = esperarAckApogeo(rssiConfirmado, snrConfirmado, ACK_TIMEOUT_MS);
#endif

          notificarApogeo(altitud_apogeo, ackRecibido, rssiConfirmado, snrConfirmado);
        }
      }
      break;
    }
