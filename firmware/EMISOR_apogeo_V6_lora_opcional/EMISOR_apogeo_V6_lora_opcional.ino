...
// El LoRa es telemetría, no es crítico para la misión: si no se detecta al
// arrancar, el vuelo debe continuar igual (detección de apogeo, buzzer,
// servo) simplemente sin poder avisar a tierra por radio.
bool loraDisponible = false;
...
void setup() {
  ...
  if (!LoRa.begin(433E6)) {
    Serial.println("¡Advertencia! No se detectó el módulo LoRa. Continuando sin telemetría...");

    // Patrón de aviso sonoro DISTINTO al del apogeo, para reconocerlo de oído
    // en la rampa: 3 pitidos cortos y agudos, en vez de un tono largo único.
    for (int i = 0; i < 3; i++) {
